...
BEGIN
  ...
  IF INSERTING THEN
    ...
    SELECT sum(VALOR_TOTAL)  FROM APUESTAS WHERE ID_USUARIO = 4 group by VALOR_TOTAL ;   
    
    UPDATE USUARIOS SET SALDO = SALDO - VALOR  WHERE ID = :new.ID_USUARIO;
    
 END IF;
 
  IF UPDATING THEN 
  
    SELECT TOTAL_GANANCIAS - VALOR_TOTAL INTO VALOR FROM APUESTAS WHERE ID_USUARIO = :old.ID_USUARIO;
    UPDATE USUARIOS SET SALDO = SALDO + VALOR  WHERE ID= :old.ID_USUARIO;
  ...
  DBMS_OUTPUT.PUT_LINE('El valor es:' || valor);
   ...
   END AFTER EACH ROW;
    ...
    BEGIN
     ...
     SELECT (TOTAL_GANANCIAS - VALOR_TOTAL) INTO VALOR  FROM (SELECT * FROM APUESTAS  ORDER BY (ID) DESC) WHERE  ROWNUM = 1;
