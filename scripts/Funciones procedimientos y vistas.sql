...
CREATE OR REPLACE PROCEDURE CREAR_DETALLE_APUESTA (V_ID_APUESTA NUMBER, V_ID_CUOTA NUMBER, V_VALOR_APOSTADO NUMBER, V_CUOTA VARCHAR2) IS
   ...
   CREATE OR REPLACE PROCEDURE PROCESAR_RETIROS (MONTO NUMBER, V_ID_USUARIO NUMBER, BANCO VARCHAR2, CUENTA VARCHAR2, V_ID_COMPROBANTE NUMBER) IS
    ...
    BEGIN
     ...
     IF SALDO_USUARIO = 0 OR SALDO_USUARIO < MONTO  THEN
          ...
          SELECT ID INTO V_ID FROM RETIROS  WHERE ID_USUARIO = V_ID_USUARIO AND ROWNUM = 1 ORDER BY (ID) DESC ;
          CONSULTA3 := ' UPDATE USUARIOS SET SALDO = SALDO + '|| MONTO||' WHERE ID = '||V_ID_USUARIO;
          DBMS_OUTPUT.PUT_LINE (CONSULTA3);
          
          CONSULTA2 := 'UPDATE RETIROS SET ESTADO = ''RECHAZADO'', OBSERVACIONES = ''NO TIENE SALDO SUFICIENTE'' WHERE ID = '||V_ID;
       

          EXECUTE IMMEDIATE CONSULTA2;
          EXECUTE IMMEDIATE CONSULTA3;
          
     ELSE 
         
          VALIDAR_COMPROBANTES(V_ID_USUARIO,MONTO);
 ...
 CREATE OR REPLACE PROCEDURE VALIDAR_COMPROBANTES(V_ID_USUARIO NUMBER ,MONTO NUMBER) 
 IS
  
  BEGIN
  
    DBMS_OUTPUT.PUT_LINE('INICIA VALIDACIÓN');
   
   DECLARE
    CURSOR VALIDAR_COMPROBATES IS  
    SELECT COM.FACTURA_SERVICIOS, COM.TAMANO_FACTURA_SERVICIOS, COM.EXTENSION_FACTURA_SERVICIOS, COM.COMPROBANTE_DEPOSITO, COM.TAMANO_COMPROBANTE_DEPOSITO, COM.EXTENSION_COMPROBANTE_DEPOSITO, COM.IDENTIFICACION, COM.TAMANO_IDENTIFICACION, COM.EXTENSION_IDENTIFICACION, COM.FOTO_PERSONAL, COM.TAMANO_FOTO_PERSONAL, COM.EXTENSION_FOTO_PERSONAL  
    FROM COMPROBANTES COM INNER JOIN RETIROS RE
    ON COM.ID = RE.ID_COMPROBANTE
    WHERE RE.ID_USUARIO = V_ID_USUARIO;
         
    FACTURA_SERVICI VARCHAR2 (255);
    TAMANO_FAC VARCHAR2 (255);
    EXTENSION_FAC VARCHAR2 (255);
    COMPRO_DEPOSI VARCHAR2 (255);
    TAMANO_COMPRO VARCHAR2 (255);
    EXTENSION_COMPRO VARCHAR2 (255);
    IDENTIFI VARCHAR2 (255);
    TAMANO_IDENTI VARCHAR2 (255);
    EXTENSION_IDENTIFI VARCHAR2 (255);
    FOTO_PERSO VARCHAR2 (255);
    TAMANO_FOTO VARCHAR2 (255);
    EXTENSION_FOTO VARCHAR2 (255);
    V_ID NUMBER;
    CONSULTA VARCHAR2 (255);
    CONSULTA3 VARCHAR2 (255);
    
    BEGIN
    OPEN VALIDAR_COMPROBATES;
        LOOP
            FETCH VALIDAR_COMPROBATES INTO FACTURA_SERVICI, TAMANO_FAC, EXTENSION_FAC, COMPRO_DEPOSI, TAMANO_COMPRO, EXTENSION_COMPRO, IDENTIFI, TAMANO_IDENTI, EXTENSION_IDENTIFI, FOTO_PERSO, TAMANO_FOTO, EXTENSION_FOTO;
            EXIT WHEN VALIDAR_COMPROBATES%NOTFOUND;    
        END LOOP;  
        
           
          IF  FACTURA_SERVICI IS NULL OR    TAMANO_FAC IS NULL OR    EXTENSION_FAC IS NULL OR    COMPRO_DEPOSI IS NULL OR  TAMANO_COMPRO IS NULL OR EXTENSION_COMPRO IS NULL OR IDENTIFI IS NULL OR TAMANO_IDENTI IS NULL OR EXTENSION_IDENTIFI IS NULL OR FOTO_PERSO IS NULL OR TAMANO_FOTO IS NULL OR EXTENSION_FOTO IS NULL
          THEN
               SELECT ID INTO V_ID FROM RETIROS  WHERE ID_USUARIO = V_ID_USUARIO AND ROWNUM = 1 ORDER BY (ID) DESC ;
               CONSULTA3 := ' UPDATE USUARIOS SET SALDO = SALDO + '|| MONTO||' WHERE ID = '||V_ID_USUARIO;
          
                SELECT ID INTO V_ID FROM RETIROS  WHERE ID_USUARIO = V_ID_USUARIO AND ROWNUM = 1 ORDER BY (ID) DESC ;
                CONSULTA := 'UPDATE RETIROS SET ESTADO = ''RECHAZADO'', OBSERVACIONES = ''NO TIENE TODOS LOS COMPROBATES'' WHERE ID = '||V_ID;
           ...
           ELSE 
                ...
                SELECT ID INTO V_ID FROM RETIROS  WHERE ID_USUARIO = V_ID_USUARIO AND ROWNUM = 1 ORDER BY (ID) DESC ;
                CONSULTA := 'UPDATE RETIROS SET ESTADO = ''APROBADO'', OBSERVACIONES = ''EXITOSO'' WHERE ID = '|| V_ID;
